...
// Ícone oficial do WhatsApp para reconhecimento imediato
...
export const Button: React.FC<ButtonProps> = ({ 
  ...
  const baseStyles = "inline-flex items-center justify-center font-extrabold py-3.5 px-6 rounded-lg transition-all duration-300 transform hover:-translate-y-0.5 shadow-md text-lg tracking-wide";
  ...
  return (
    ...
    >
      {variant === 'whatsapp' ? (
        <WhatsAppLogo className="w-6 h-6 mr-2.5 fill-current" />
      ) : (
        variant === 'primary' && <Phone className="w-5 h-5 mr-2 fill-current" />
      )}
      {children}
    </button>
  );
